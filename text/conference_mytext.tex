...
\begin{equation*}
	\mathbf{s}_{\mathrm{R}} = M_{\mathrm{R}}\mathbf{s},
\end{equation*}
where $  M_{\mathrm{R}} $ is identity matrix $M_{\mathbb{R}} \in R ^ { |I^R| \times N}$ ($ |I^R|$ as length of subset) \cite{Adler2012}.
...
The closest redefinition is to use the l1 norm as follows:
...
\begin{algorithm}
	...
	\end{algorithmic} 
\end{algorithm}
In \ref{alg:DRA_c} with $ \operatorname{proj}_{\Gamma} $ is projection onto convex set $ \Gamma $.


\subsection{SAMPLES as INPUT APPROACH}\label{subsec:timecoef}

\begin{equation*}
	\mathop {\operatorname{arg \, min}}_\mathbf {s}\Vert \mathbf {s}\Vert _1 \quad \text{s.t.}\ D\mathbf {s}\in \Gamma, \tag{3}
\end{equation*} 

\begin{equation*}
	\Gamma = \lbrace \mathbf {z}\in \mathbb {C}^L\mid M_{\mathrm{R}}\mathbf {z}=M_{\mathrm{R}}\mathbf {y}\rbrace, \tag{4} 
\end{equation*}
...
Projection, in this case, is simply replacing reconstructed samples with samples considered reliable.
